...
pub trait RigDriverExt: RigDriver {
    /// Visit all the variables skipping the Unit ones
    fn for_each_parameter(&self, mut visitor: impl FnMut(&dyn Variable) -> bool) {
        self.visit_variables(&mut |v| if v.kind() != ValueKind::Unit { visitor(v) } else { true });
    }

    /// Return the variable by name skipping the Unit ones
    fn set_parameter_value(&mut self, name: &str, value: ValueType) -> Result<(), RigError> {
        let param = self
            .variable_mut(name)
            .filter(|v| v.kind() != ValueKind::Unit)
