use crate::{
    input_manager::{
        ActionLike, DetectedGesture, InputDriver, InputDrivers, InputMap, InputProcessor, TypedInputProcessor,
    },
...
impl InputDriver for DetectedGesture {}

pub fn integrate_gesture_inputs<A>(
    recognizer_query: Query<(Entity, &DetectedGesture)>,
    ...
    A: ActionLike,
{
    ...
    for (entity, detected_gesture) in recognizer_query.iter() {
...
impl InputProcessor for GestureInput {
    ...
    fn integrate(&mut self, input: &InputDrivers) {
        if input.has_marker::<DetectedGesture>() {
            // When gesture input is available, reset the pressed state
            // and see if any detector has the given gesture.
            self.pressed = false;
...
impl TypedInputProcessor<bool> for GestureInput {
