...
/// Handles positional input for unistroke gesture recognition.
/// Collects input positions until a `None` value is encountered, then classifies
/// the collected points against gesture templates using the Jackknife algorithm.
/// Recognition results are available one frame after the input is completed.
#[derive(Component)]
