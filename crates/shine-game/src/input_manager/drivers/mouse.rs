use crate::input_manager::{ActionLike, InputDriver, InputDrivers, InputMap, InputProcessor, TypedInputProcessor};
use bevy::{
    ecs::{
        error::BevyError,
        system::{Query, Res},
...
impl InputDriver for ButtonInput<MouseButton> {}
impl InputDriver for AccumulatedMouseMotion {}
    ...
    A: ActionLike,
{
    ...
    let mut input_sources = InputDrivers::new();
...
impl InputProcessor for MouseButtonInput {
...
impl TypedInputProcessor<bool> for MouseButtonInput {
...
impl InputProcessor for MouseMotion {
    fn name(&self) -> Cow<'_, str> {
...
impl TypedInputProcessor<Vec2> for MouseMotion {
...
impl InputProcessor for MousePosition {
    fn name(&self) -> Cow<'_, str> {
        self.name.as_deref().unwrap_or("").into()
    }
...
impl TypedInputProcessor<Vec2> for MousePosition {
    fn process(&mut self, _time: f32) -> Option<Vec2> {
        self.value
    }
}
