use crate::math::value::{
    Animated, Animation, CurveAnimation, IdentityAnimate, Interpolate, MapAnimation, PredictAnimation, SmoothAnimation,
    TimeAnimation, TweenLike, ValueError, ValueKind, ValueLike, ValueType, Variable,
};
use bevy::math::{Curve, Quat, Vec2, Vec3, Vec4};
use std::borrow::Cow;

pub struct AnimatedVariable<T, U = T, A = IdentityAnimate<T>>
where
    T: TweenLike,
    U: TweenLike,
...
impl<T> Default for AnimatedVariable<T>
where
    T: TweenLike + Default,
{
    fn default() -> Self {
        AnimatedVariable::new(T::default(), T::default())
    }
}

impl<T> AnimatedVariable<T, T, IdentityAnimate<T>>
where
    T: TweenLike,
    T: TweenLike,
{
...
impl AnimatedVariable<(), f32, TimeAnimation> {
    pub fn time() -> Self {
...
impl<T, U, A> AnimatedVariable<T, U, A>
where
    T: TweenLike,
    U: TweenLike,
    A: Animation<In = T, Out = U>,
...
impl<T, U, A> Variable for AnimatedVariable<T, U, A>
where
...
impl<T, U, A> Animated for AnimatedVariable<T, U, A>
where
    T: TweenLike + Variable,
    U: TweenLike,
    A: Animation<In = T, Out = U>,
...
where
    ...
    A: Animation<In = T, Out = U>,
{
    pub fn smooth(self, duration_s: f32) -> AnimatedVariable<T, U, impl Animation<In = T, Out = U>>
    where
        U: Interpolate,
    {
    ...
    pub fn predict(self, duration_s: f32) -> AnimatedVariable<T, U, impl Animation<In = T, Out = U>>
    where
        U: Interpolate,
    {
    ...
    ) -> AnimatedVariable<T, V, impl Animation<In = T, Out = V>>
    where
        V: TweenLike,
    {
    ...
    pub fn boxed(self) -> AnimatedVariable<T, U, Box<dyn Animation<In = T, Out = U>>>
    where
        U: Interpolate,
    {
        AnimatedVariable {
...
impl<T, A> AnimatedVariable<T, f32, A>
where
    T: TweenLike,
    A: Animation<In = T, Out = f32>,
{
    pub fn curve<V, C>(self, curve: C) -> AnimatedVariable<T, V, impl Animation<In = T, Out = V>>
    where
        V: TweenLike,
...
pub trait IntoAnimatedVariable: Variable {
    fn animated(self) -> AnimatedVariable<Self>
    where
        Self: Sized + ValueLike,
    {
        AnimatedVariable::new(self.clone(), self)
    }
}
