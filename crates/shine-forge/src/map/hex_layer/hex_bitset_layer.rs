use crate::map::{AxialCoord, HexDenseIndexer, HexLayer, HexLayerConfig, MapLayer, Tile};
use bevy::ecs::component::Component;
use fixedbitset::FixedBitSet;
use std::marker::PhantomData;

/// A 2d hexagonal grid of bitset.
/// The T tile generic parameter is used to scope the layer to a specific tile type, but the actual data is just a bitset.
#[derive(Component)]
pub struct HexBitsetLayer<T>
where
    T: Tile,
{
    ...
    T: Tile,
{
    ...
    pub fn get(&mut self, coord: AxialCoord) -> bool {
    ...
    pub fn set(&mut self, coord: AxialCoord) {
        if self.is_in_bounds(coord) {
            let index = self.indexer.get_dense_index(&coord);
    ...
    pub fn ones(&self) -> impl Iterator<Item = AxialCoord> + '_ {
    ...
    pub fn zeroes(&self) -> impl Iterator<Item = AxialCoord> + '_ {
        self.data.zeroes().map(move |index| self.indexer.get_coord(index))
    }
}

impl<T> MapLayer for HexBitsetLayer<T>
where
    T: Tile,
{
    ...
    fn initialize(&mut self, config: &Self::Config) {
        self.indexer = HexDenseIndexer::new(config.radius);
...
impl<T> HexLayer for HexBitsetLayer<T>
where
    T: Tile,
{
    fn radius(&self) -> u32 {
