...
/// A shard of a hexagonal grid of tiles where tiles are usually
...
impl<T> MapShard for HexSparseShard<T>
where
    T: Tile,
{
    type Tile = T;
    ...
    type Primary = HexSparseLayer<T>;
    type Overlay = HexSparseLayer<T>;
    type Audit = HexBitsetLayer<T>;
}

impl<T> Plugin for HexSparseShard<T>
where
    T: Tile,
{
    fn build(&self, app: &mut App) {
