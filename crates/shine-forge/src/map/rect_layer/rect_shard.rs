...
/// A shard of a rectangular grid of tiles where tiles are usually
/// different from each other and have a unique value.
...
/// A shard of a rectangular grid of tiles where tiles are usually
/// homogeneous with a few different values.
pub struct RectSparseShard<T>
where
    T: Tile,
{
    system_config: MapShardSystemConfig<Self>,
...
impl<T> MapShard for RectSparseShard<T>
where
    T: Tile,
{
    type Tile = T;
    ...
    type Primary = RectSparseLayer<T>;
    type Overlay = RectSparseLayer<T>;
    type Audit = RectBitsetLayer<T>;
}

impl<T> Plugin for RectSparseShard<T>
where
    T: Tile,
{
    fn build(&self, app: &mut App) {
