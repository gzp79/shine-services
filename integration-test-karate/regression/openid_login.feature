Feature: OpenId flow
  ...
  Scenario: Auth (parameters: NO, cookie: NO) should be an error
    Given def mock = karate.start({mock:'../mocking/openid.feature', port: port})

    Given path '/auth/openid_flow/auth'
    ...
    Then status 200
      ...
      * match response contains 'Failed to deserialize query string'

  Scenario: Auth (parameters: VALID, cookie: NO) should be an error
    Given def mock = karate.start({mock:'../mocking/openid.feature', port: port})
    
    Given path '/auth/openid_flow/login'
      * params utils.defaultRedirects
      * configure cookies = null
      * method get
    Then status 200
      * match utils.getRedirectUrl(response) contains 'http://mock.localhost.com:8090/openid/authorize'
  ...
  Scenario: Auth (unreachable 3rd party) should be an error 
    ...
    # mockserver is required for the login to finish discovery, but before auth it is stopped
    Given def mock = karate.start({mock:'../mocking/openid.feature', port: port})
    Given path '/auth/openid_flow/login'
      * params utils.defaultRedirects
      * configure cookies = null
