...
export class RequestAPI {
    ...
    validate(tid: string | null, sid: string | null, eid: string | null) {
        ...
        return request
            .get(this.config.getIdentityUrlFor('auth/validate'))
            .set('Cookie', [...ct, ...cs, ...ce]);
    }
    ...
    loginWithToken(
        ...
        return request
            .get(this.config.getIdentityUrlFor('auth/token/login'))
    ...
    loginWithOAuth2(
        ...
        return request
            .get(config.getIdentityUrlFor('auth/oauth2_flow/login'))
    ...
    linkWithOAuth2(sid: string | null): Request {
        return request
            .get(config.getIdentityUrlFor('auth/oauth2_flow/link'))
    ...
    authorizeWithOAuth2(
        ...
        return request
            .get(config.getIdentityUrlFor('auth/oauth2_flow/auth'))
            .query({ ...qs, ...qc })
            .set('Cookie', [...cs, ...ce]);
    }
    ...
    loginWithOpenId(
        ...
        return request
            .get(config.getIdentityUrlFor('auth/openid_flow/login'))
            .query({ ...qs, ...qc, ...config.defaultRedirects })
            .set('Cookie', [...ct, ...cs]);
    }
    ...
    linkWithOpenId(sid: string | null): Request {
        return request
            .get(config.getIdentityUrlFor('auth/openid_flow/link'))
            .query({ ...config.defaultRedirects })
            .set('Cookie', sid ? [`sid=${sid}`] : []);
    }
    ...
    authorizeWithOpenId(
        ...
        return request
            .get(config.getIdentityUrlFor('auth/openid_flow/auth'))
            .query({ ...qs, ...qc })
            .set('Cookie', [...cs, ...ce]);
    }
    ...
    logout(sid: string | null, terminateAll: boolean | null): Request {
        return request
            .get(config.getIdentityUrlFor('/auth/logout'))
            .query({ terminateAll })
            .set('Cookie', sid ? [`sid=${sid}`] : []);
    }
    ...
    getUserInfo(sid: string | null): Request {
        return request
            .get(config.getIdentityUrlFor('api/auth/user/info'))
            .set('Cookie', sid ? [`sid=${sid}`] : []);
    }
    ...
    getRoles(sid: string | null, masterKey: boolean, userId: string): Request {
        ...
        return request
            .get(config.getIdentityUrlFor(`/api/identities/${userId}/roles`))
            .set('Cookie', sid ? [`sid=${sid}`] : [])
    ...
    addRole(sid: string | null, masterKey: boolean, userId: string, role: string): Request {
        ...
        return request
            .put(config.getIdentityUrlFor(`/api/identities/${userId}/roles`))
            .set('Cookie', sid ? [`sid=${sid}`] : [])
            .set(av ? { Authorization: av } : {})
            .type('json')
            .send({ role: role });
    ...
    deleteRole(sid: string | 'masterKey' | null, masterKey: boolean, userId: string, role: string): Request {
        ...
        return request
            .delete(config.getIdentityUrlFor(`/api/identities/${userId}/roles`))
            .set('Cookie', sid ? [`sid=${sid}`] : [])
            .set(av ? { Authorization: av } : {})
            .type('json')
            .send({ role: role });
    ...
    getSessions(sid: string | null): Request {
        return request
            .get(config.getIdentityUrlFor('api/auth/user/sessions'))
            .set('Cookie', sid ? [`sid=${sid}`] : []);
    }

    getTokens(sid: string | null): Request {
        return request
            .get(config.getIdentityUrlFor('api/auth/user/tokens'))
            .set('Cookie', sid ? [`sid=${sid}`] : []);
    }

    getToken(sid: string | null, tokenId: string): Request {
        return request
            .get(config.getIdentityUrlFor(`api/auth/user/tokens/${tokenId}`))
            .set('Cookie', sid ? [`sid=${sid}`] : []);
    }

    revokeToken(sid: string | null, tokenId: string): Request {
        return request
            .delete(config.getIdentityUrlFor(`api/auth/user/tokens/${tokenId}`))
            .set('Cookie', sid ? [`sid=${sid}`] : []);
    }

    createToken(sid: string | null, kind: TokenKind, duration: number, bindToSite: boolean): Request {
        return request
            .post(this.config.getIdentityUrlFor('api/auth/user/tokens'))
            .set('Cookie', sid ? [`sid=${sid}`] : [])
            .type('json')
            .send({
                kind,
    ...
    getExternalLinks(sid: string | null): Request {
        return request
            .get(config.getIdentityUrlFor('api/auth/user/links'))
            .set('Cookie', sid ? [`sid=${sid}`] : []);
    }
    ...
    unlink(sid: string | null, provider: string, providerUserId: string): Request {
        ...
        return request.delete(config.getIdentityUrlFor(url)).set('Cookie', sid ? [`sid=${sid}`] : []);
    }
}
