...
test.describe('CORS check', { tag: ['@regression'] }, () => {
    ...
    test('Allow origin shall be present for not-found request', async ({ identityUrl, api: { client } }) => {
    ...
    test('Allow origin shall be present for a successful request', async ({ identityUrl, api: { client } }) => {
        const url = joinURL(identityUrl, '/info/ready');
        const originUrl = new URL(identityUrl);
        const origin = `${originUrl.protocol}//${originUrl.hostname}:${originUrl.port}`;
        const response = await client.get(url).withHeaders({ Origin: origin });
    ...
    test('Allow origin shall be present for any subdomain', async ({ identityUrl, api: { client } }) => {
        const url = joinURL(identityUrl, '/info/ready');
        const originUrl = new URL(identityUrl);
    ...
    test('Allow origin shall be present for another port', async ({ identityUrl, api: { client } }) => {
        const url = joinURL(identityUrl, '/info/ready');
        const originUrl = new URL(identityUrl);
    ...
    test('Allow origin shall not be present for another protocol', async ({ identityUrl, api: { client } }) => {
        const url = joinURL(identityUrl, '/info/ready');
        const originUrl = new URL(identityUrl);
    ...
    test('Allow origin shall not be present for another domain', async ({ identityUrl, api: { client } }) => {
        const url = joinURL(identityUrl, '/info/ready');
