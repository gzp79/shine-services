...
describe('InstanceBuffer.grow', () => {
    it('doubles capacity and preserves existing slot data', () => {
        ...
        buf.copyToBuffer(10, 0, new Float32Array([1, 2, 3, 4]));
        buf.copyToBuffer(20, 0, new Float32Array([5, 6, 7, 8]));
    ...
    it('marks all buffers dirty after grow', () => {
        const buf = new InstanceBuffer(2, [1]);
        buf.copyToBuffer(1, 0, new Float32Array(4));
        buf.compact();
        buf.clearDirty(0);
        buf.grow(4);
    ...
    it('compact() returns true when DataTextures were grown', () => {
        const buf = new InstanceBuffer(2, [1]);
        buf.copyToBuffer(0, 0, val(0));
        buf.copyToBuffer(1, 0, val(1));
        expect(buf.compact()).toBe(false);
        buf.clearDirty(0);
        // overflow into CPU-only territory
        buf.copyToBuffer(2, 0, val(2));
...
describe('InstanceBuffer', () => {
    describe('set / remove basics', () => {
        it('copyToBuffer adds a key and compact updates length', () => {
            const buf = buf1(4);
            buf.copyToBuffer(1, 0, val(1));
            buf.compact();
        ...
        it('copyToBuffer same key twice updates data in place', () => {
            const buf = buf1(4);
            buf.copyToBuffer(1, 0, val(1));
            buf.copyToBuffer(1, 0, new Float32Array([99, 99, 99, 0]));
            expect(buf.length).toBe(1);
            buf.compact();
        ...
        it('copyToBuffer auto-grows CPU capacity when full', () => {
            const buf = buf1(2);
            buf.copyToBuffer(0, 0, val(0));
            buf.copyToBuffer(1, 0, val(1));
            // No longer returns false — CPU grows automatically
            expect(buf.copyToBuffer(2, 0, val(2))).toBe(true);
    ...
    describe('compact', () => {
        ...
        it('multiple gaps — compact leaves no holes in active range', () => {
            const buf = buf1(8);
            for (let i = 0; i < 8; i++) buf.copyToBuffer(i, 0, val(i));
            buf.remove(1);
            buf.remove(3);
            buf.remove(5);
            buf.remove(7);
        ...
        it('fill, remove all, re-fill — slots are reused correctly', () => {
            const buf = buf1(4);
            for (let i = 0; i < 4; i++) buf.copyToBuffer(i, 0, val(i));
            for (let i = 0; i < 4; i++) buf.remove(i);
            buf.compact();
            for (let i = 10; i < 14; i++) buf.copyToBuffer(i, 0, val(i));
            buf.compact();
            expect(buf.length).toBe(4);
        ...
        it('gaps at both ends and in the middle', () => {
            const buf = buf1(6);
            for (let i = 0; i < 6; i++) buf.copyToBuffer(i, 0, val(i));
            buf.remove(0);
            buf.remove(2);
            buf.remove(5);
            buf.compact();
    ...
    describe('dirty tracking', () => {
        it('copyToBuffer marks the target buffer dirty', () => {
            const buf = new InstanceBuffer(4, [1, 1]);
            buf.copyToBuffer(0, 1, new Float32Array([1, 2, 3, 4]));
        ...
        it('compact after remove marks all buffers dirty', () => {
            const buf = new InstanceBuffer(4, [1, 1]);
            buf.copyToBuffer(0, 0, new Float32Array([1, 2, 3, 0]));
            buf.copyToBuffer(0, 1, new Float32Array([1, 2, 3, 4]));
            buf.clearDirty(0);
            buf.clearDirty(1);
            buf.remove(0);
    ...
    describe('multiple buffers', () => {
        it('independent buffers hold correct data', () => {
            const buf = new InstanceBuffer(4, [1, 1]);
            buf.copyToBuffer(0, 0, new Float32Array([1, 2, 3, 0]));
            buf.copyToBuffer(0, 1, new Float32Array([7, 0, 0, 0]));
            buf.compact();
        ...
        it('moveSlot copies all buffers correctly', () => {
            const buf = new InstanceBuffer(4, [1, 1]);
            buf.copyToBuffer(0, 0, new Float32Array([10, 20, 30, 0]));
            buf.copyToBuffer(0, 1, new Float32Array([100, 0, 0, 0]));
            buf.copyToBuffer(1, 0, new Float32Array([11, 21, 31, 0]));
            buf.copyToBuffer(1, 1, new Float32Array([110, 0, 0, 0]));
